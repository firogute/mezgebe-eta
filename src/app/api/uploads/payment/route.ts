...
export async function POST(request: Request) {
  try {
    ...
    const objectPath = `payment-receipts/${fileName}`;
    ...
    if (uploadError) {
      console.error("Supabase payment receipt upload error", uploadError);
      return NextResponse.json(
        { error: "Failed to upload payment receipt image." },
        { status: 500 },
      );
    }
    ...
    console.error("Failed to upload payment receipt image", error);
    return NextResponse.json(
      { error: "Failed to upload payment receipt image." },
      { status: 500 },
    );
